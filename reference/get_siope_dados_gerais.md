...
columns including `tipo`, `num_ano`, `sig_uf`, `cod_muni`, `nom_muni`,
`num_popu`, revenue and expense values, PIB data, and declaration
metadata.

## Details

**Performance tip**: use the `filter` parameter to narrow results on the
server before downloading. For example, filtering by municipality name
or IBGE code returns only the rows you need instead of all
